...
public class MinersHideoutmainFeature extends Feature<NoneFeatureConfiguration> {
	public static MinersHideoutmainFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new MinersHideoutmainFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("portdiscs:miners_hideout", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("portdiscs:miners_hideout", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public MinersHideoutmainFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("portdiscs", "minershideoutv2_main"));
		if (template == null)
			return false;
