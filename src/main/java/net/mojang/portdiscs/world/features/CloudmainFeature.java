...
public class CloudmainFeature extends Feature<NoneFeatureConfiguration> {
	public static CloudmainFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new CloudmainFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("portdiscs:cloud", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("portdiscs:cloud", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("portdiscs", "cloud_main"));
		if (template == null)
			return false;
		boolean anyPlaced = false;
		if ((context.random().nextInt(1000000) + 1) <= 620) {
